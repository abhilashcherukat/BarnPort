...
<html lang="en">
	...
	<?php
		if(isset($_POST['OK_btn_CreateFormPopup_AddcourseType']))
		{
			...
			$params=['opt'=>'1','title'=>$_POST['courseType_title'],'id'=>$_POST['AddcourseType_IdValue']];
			$post_url = $URL."/commonlist/course";
			$post_response=$Obj_Commonfunction->CurlSendPostRequest($post_url,$params) ;
		}
		if(isset($_POST['OK_btn_CreateFormPopup_EditcourseType']))
		{
			$params=['opt'=>'2','title'=>$_POST['courseType_title'],'id'=>$_POST['AddcourseType_IdValue']];
			$post_url = $URL."/commonlist/course";
			$post_response=$Obj_Commonfunction->CurlSendPostRequest($post_url,$params) ;
	...
	?>
	 <?php require_once('navbar_include.php'); ?>
					...
					<div class="row">
						<div class="col-md-12">
							...
							<div class="row padding-top">
								<div class='btn btn-success circle' id='btn_addcourseType' data-toggle='modal' data-target='#AddcourseType' data-original-title onclick='Addnew()' >Add new</div>
								<div class="col-md-12" id='container'>
									...
									?>
									<table class='table' id='courseTypetbl'>
										<tr>
										<th>#</th>
										<th>Title</th>
										<th></th>
		...
		<?php require_once('Modal.php'); 
			CreateConfirmModal("Delete courseType","Are you sure you want to delete <b id='currentObject'>this</b>?");
			...
			$Html="  <div class='row'>
						...
						<div class='col-xs-6 col-sm-6 col-md-3'>
							<div class='form-group'>
								<input type='text' name='courseType_title'  id='courseType_title' class='form-control input-sm' required>
							</div>
						</div>
			...
			CreateFormPopup("AddcourseType","Add new courseType",$Html,'AddcourseType',"?page=".$Page,"enctype= multipart/form-data");
		?>
	</body>
	<?php 
	...
	<script>
		$(document).ready(function()
		{
			...
			var posting=$.ajax({ 	
										type: "GET",
										url: "<?php echo $URL; ?>/commonlist/course",
			...
			posting.done(function(data)
			{
				...
				}else
				{
					...
					}else
					{
							...
							{
								$('#courseTypetbl').append("<tr><td>No records</td></tr>");
							}
	...
	function Addnew()
	{
		$('#TIT_AddcourseType').html("Add");
		$('#courseType_title').val("");
		$('#OK_btn_CreateFormPopup').attr('name','OK_btn_CreateFormPopup_AddcourseType');
	}
	function Editthis(Id)
	{
		$('#TIT_AddcourseType').html("Edit");
		var posting=$.ajax({ 	
										type: "GET",
										url: "<?php echo $URL; ?>/commonlist/course",
										<?php if(isset($_SESSION['USERID'])){$USERID=$_SESSION['USERID'];}else{$USERID="";} 	?>
			...
			posting.done(function(J) {
				clog(J)
			$('#courseType_title').val(J.data.title);
			$('#opt').val(2);
			$('#AddcourseType_IdValue').val(Id);
			$('#OK_btn_CreateFormPopup').attr('name','OK_btn_CreateFormPopup_EditcourseType');
			});
